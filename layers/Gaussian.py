...
class Gaussian:
    ...
    def ddpm_sample(self, diff, *conditions, embeddings, clip_denoised=True, desc=None):
        diff.eval()
        sample_images = torch.randn_like(conditions[0])
        first_step = True
        for t in tqdm(reversed(range(0, self.time_steps)), desc=desc, total=self.time_steps, unit='step', ncols=80):
            ...
            sample_images = sqrt_recip_alphas_t * (
                    sample_images - betas_t * diff(sample_images, t, *conditions, embeddings=embeddings,
                                                   first_step=first_step) / sqrt_one_minus_alphas_cumprod_t)
    ...
    def ddim_sample(self, diff, *conditions, embeddings, ddim_timesteps=30,
                    ddim_discr_method='uniform', ddim_eta=0.0, clip_denoised=True, desc=None):
        diff.eval()
        ...
        sample_images = torch.randn_like(conditions[0])
        first_step = True
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc=desc, total=ddim_timesteps, unit='step', ncols=80):
            t = torch.full((sample_images.shape[0],), int(ddim_timestep_seq[i]), device=self.device, dtype=torch.long)
            prev_t = torch.full((sample_images.shape[0],), int(ddim_timestep_prev_seq[i]), device=self.device,
                                dtype=torch.long)
